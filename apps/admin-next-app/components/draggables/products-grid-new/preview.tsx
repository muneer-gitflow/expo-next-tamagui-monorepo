...
interface ProductsGridNewPreviewProps {
  blockId: string;
  block: UIBlock;
}

export default function ProductsGridNewPreview({ blockId, block }: ProductsGridNewPreviewProps) {
  return (
    <InlineGrid gap="200">
        ...
        <InlineGrid gap={'300'}>
          <InlineStack gap={'300'}>
            {Array.from({ length: 2 }).map((_, i) => (
              <Box width="calc(50% - 6px)" key={String(i)}>
                <InlineGrid gap={'100'}>
                  <span
                    style={{
                      minWidth: '100%',
